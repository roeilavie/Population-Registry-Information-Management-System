...
void init_db(db_mgr* p) // מקצה מערך דינאמי למספר התושבים
{
	if (p->realTimeCitizen == 0)
	{
		scanf("%d", &p->actualcitizen);
		while (p->actualcitizen < 0)
		{
			printf("Error, please enter number bigger or even to 0\n");
			scanf("%d", &(p->actualcitizen));
		}
	}
...
void checkChildren(Person* newPerson)// פונקציית עזר בדיקה ילדים
{
	...
	do { // בדיקה שהוכנס מספר ילדים תקין
		...
		case 1:
			...
			while (newPerson->NumofChildren < 0)
			{
...
void get_gen(db_mgr* citizen) //מחשבת דורות
{
	...
	while (len > 0)
	{
		...
		if (curr_child == NULL || (int)curr_child->NumofChildren == 0)
		{
...
int get_left_len(db_mgr* citizen, Person* p) // בדיקת מסלול שמאלי של עץ הדורות
{
	...
	while (child && (int)child->NumofChildren > 0)//  מעלים את המשתנה הזמני ב1 כל עוד לא קיים ילד לילד
	{
		max++;
...
void append(Person_LVL** arr, long long* ids, int* arrsize, int childrenCount, int lvl)// 
{
	...
	for (int i = 0; i < *arrsize; i++) // מעתיקים את ARR לתוך RES .
	{
		res[i] = (*arr)[i];
	}
	free(*arr);

	for (int i = 0, j = *arrsize; i < childrenCount; i++, ++j) // שירשור את רשימת תז הילדים לתוך RES .
	{
		Person_LVL p;
		p.lvl = lvl;
		p.person_id = ids[i];
